...
public class POPClassProcessor extends AbstractProcessor {
	...
	private void processClass(Element element) {
		...
		if (popAnnotation == null) {
			messager.printMessage(Diagnostic.Kind.ERROR,
					"POP-Class " + element + " did not have the @" + POPClass.class.getName() + " annotation", element);
			return;
		}
		...
		for (Element subelement : element.getEnclosedElements()) {
			if (subelement.getKind() == ElementKind.METHOD && subelement.getModifiers().contains(Modifier.PUBLIC)) {
				...
				// TODO: do a basic check if all parameters of the method can be
				// serialized
				/*
				 * if(!areParametersSerializable(subelement)){
				 * messager.printMessage(Diagnostic.Kind.ERROR,
				 * "Can not serialize all parameters of "+subelement, subelement); }
				 */
			}
		}
	}
	...
	private void checkPublicMethodAnnotation(Element method) {
		...
		if (method.getAnnotation(POPAsyncSeq.class) == null && method.getAnnotation(POPAsyncConc.class) == null
				&& method.getAnnotation(POPAsyncMutex.class) == null && method.getAnnotation(POPSyncSeq.class) == null
				&& method.getAnnotation(POPSyncConc.class) == null && method.getAnnotation(POPSyncMutex.class) == null
				&& method.getAnnotation(POPPrivate.class) == null) {
			messager.printMessage(Diagnostic.Kind.WARNING,
					"Method " + method + " is public but does not have a POP-Java annotation, @"
							+ POPSyncConc.class.getSimpleName() + " will be used",
					method);
			return;
		}
	...
	private static final TypeVisitor<Boolean, Void> SERIALIZABLE_ARGS_VISITOR = new SimpleTypeVisitor6<Boolean, Void>() {
		...
		public Boolean visitExecutable(ExecutableType t, Void v) {
			for (TypeMirror type : t.getParameterTypes()) {
				...
				// Sadly it is not possible to get the class of the parameter
				// due to the way annotation preprocessors work
			}
			return true;
		}
	};
