...
/**
 * This class starts a daemon that listens to incoming POP requests. It replaces
 * the SSH service to start brokers.
 * 
 * @author Beat Wolf
 *
 */
public class POPJavaDeamon implements Runnable, Closeable {
	 ...
	 * @param encrypt
	 *            if we are encrypting or decrypting
	...
	public static Cipher createKey(String salt, String password, boolean encrypt) {
		try {
			...
			KeySpec spec = new PBEKeySpec(password.toCharArray(), salt.getBytes(), 65536,
					Math.min(Cipher.getMaxAllowedKeyLength("AES"), 256));
	...
	/**
	 * Class that handles the accepted connections and runs the Broker with the
	 * provided paramters
	 * 
	 * @author Beat Wolf
	 *
		...
		public void run() {
			...
			try {
				...
				if (!saltedHash.equals(challengeAnswer)) {
					System.err.println("The supplied secret was wrong : " + challengeAnswer + " should be " + saltedHash
				...
				for (int i = 0; i < commandLength; i++) {
					...
					// If the current parameter is the classpath, modify it to
					// fit local system
