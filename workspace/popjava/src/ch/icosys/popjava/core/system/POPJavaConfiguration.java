...
public class POPJavaConfiguration {
	...
	public static String getPopAppCoreService() {
		...
		if (appCoreService == null) {
			...
			// return service;
			// appCoreService = "gdb -ex=run --args
			// /usr/local/popc/services/appservice";
			// appCoreService = "valgrind --log-file=/home/asraniel/valgrind.txt
			// /usr/local/popc/services/appservice";
	...
	public static String getClassPath() {
		...
		return path;
		/*
		 * StringBuilder popJar = new StringBuilder(); Set<String> paths = new
		 * HashSet<>();
		 * 
		 * if(POPAppService.class.getClassLoader() instanceof URLClassLoader) { URL []
		 * urls = ((URLClassLoader)POPAppService.class.getClassLoader()).getURLs();
		 * 
		 * for(int i = 0; i < urls.length; i++){ URL url = urls[i]; try { String path =
		 * new File(url.toURI()).getAbsolutePath(); paths.add(path); } catch
		 * (URISyntaxException e) { e.printStackTrace(); } } }else { return
		 * System.getProperty("java.class.path"); }
		 * 
		 * List<String> pathList = new ArrayList<>(paths);
		 * 
		 * for(int i = 0; i < pathList.size(); i++){ popJar.append(pathList.get(i));
		 * if(i != pathList.size() - 1){ popJar.append(File.pathSeparatorChar); } }
		 * 
