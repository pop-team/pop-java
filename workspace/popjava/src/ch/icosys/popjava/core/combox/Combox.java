 ...
 * 
 * @param <T>
 *            The "real" connection of the Combox, be it a Socket or a Pipe.
 */
public abstract class Combox<T> {
	...
	private Broker broker = null; // Broken to be used when making this combox
	// bidirectional
	 ...
	 * 
	 * @param networkUUID
	 *            The network UUID that will be send to the other end
	 */
	 ...
	 * 
	 * @param broker
	 *            Broker that is behind this connection
	 * @param accesspoint
	 *            Access point of the other side combox
	 * @param timeout
	 *            Connection time out
	...
	/**
	 * Accept a connection from {@link #connectToServer() }. Communicate with Client
	 * mode Combox.
	 * 
	...
	/**
	 * Called by the client, it send the name of the network its in. This must use
	...
	/**
	 * Server accept connection. Usually setup T I/O streams. NOTE you should NEVER
	 * use this method directly, use {@link #serverAccept(java.lang.Object) }
	 * 
	...
	public int makeBidirectional(Broker broker) {
		...
		if (this.broker == null) {
			...
			// System.out.println("Cannot two different brokers for same
			// combox!");
		}
		...
		if (answer > 0) {
			if (buffer.getHeader().getRequestType() == MessageHeader.EXCEPTION) {
				// System.out.println("Bidirectional connection failed,
				// exception "+buffer.getHeader().getExceptionCode()+" "+this);
				return -1;
			}
			int connectionID = buffer.getInt();
			;
			// System.out.println("Combox accepted bidirectional on connection
			// "+connectionID+" "+this);
	...
	protected boolean bindToBroker(int connectionID) {
		// System.out.println("Rebind combox to broker using connection ID
		// "+connectionID+" "+this);
		if (broker != null) {
	...
	protected void handleComboxMessages(POPBuffer tempBuffer) {
		...
		case OPEN_BIDIRECTIONAL: {// Handle the opening of a bidirectional
			// channel
