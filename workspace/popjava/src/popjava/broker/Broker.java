...
public class Broker {
	 ...
	 * @param codelocation 	path of the real object to create with this broker
	...
	/**
	 * This method is responsible to invoke the right constructor on the associated object
	 * @param request	Request received from the interface-side
	...
	private boolean invokeConstructor(Request request) {
		...
		if (exception == null) {
			try {
				...
				exception = POPException.createReflectException(constructor
	...
	/**
	 * This method is responsible to call the correct method on the associated object
	 * @param request	Request received from the interface-side
	 * @return true if the method has been called correctly
	 */
	private boolean invokeMethod(Request request) {
		...
		if (exception == null) {
			...
			for (index = 0; index < parameterTypes.length; index++) {
				...
				try {					
					...
					LogWriter.writeDebugInfo(parameterTypes[index].getName() + " " + parameters[index].toString());
				} catch (POPException e) {
					exception = new POPException(e.errorCode, e.errorMessage);
					break;
				} catch (Exception e) {
		...
		if (exception == null) {
			try {
				...
				if (returnType != Void.class && returnType != void.class){
					...
					LogWriter.writeDebugInfo("Result is : "+result.toString());
				} else {
				...
				exception = POPException.createReflectException(method
						.getName(), e.getMessage());
			...
			if ((request.getSenmatics() & Semantic.Synchronous) != 0) {
				...
				for (index = 0; index < parameterTypes.length; index++) {
					try {
						...
						LogWriter.writeDebugInfo("Execption serialize parameter");
						exception = new POPException(e.errorCode,
								e.errorMessage);
						break;
					}
				...
				if (exception == null) {
					if (returnType != Void.class && returnType != void.class
						...
						try {
							LogWriter.writeDebugInfo("fill buffer"+result.toString());
	...
	public boolean popCall(Request request) {
		...
		case MessageHeader.AddRefCall: {
			// AddRef call...
			LogWriter.writeLogInfo("AddRef"+popObject.toString(),"/tmp/mylog" );
			if (popInfo == null) {
				return false;
			}
			int ret = 1;
		...
		case MessageHeader.DecRefCall: {
			LogWriter.writeLogInfo("DecRef"+popObject.toString(),"/tmp/mylog" );
			// DecRef call....
			if (popInfo == null) {
				return false;
			}
	 ...
	 * @param state	state to set to this broker
	 */
	 ...
	 * @param encoding	Encoding to look for
	...
	public boolean initialize(ArrayList<String> argvs) {
		...
		for (int i = 0; i < comboxCount; i++) {
			...
			AccessPoint ap = new AccessPoint(factory.getComboxName(), POPSystem
					.getHost(), iPort);
	 ...
	 * @param urlClassLoader	Path of the class
	 ...
	 * @param argvs arguments of the program
	 */
	 ...
	 * @param combox	Combox to send the exception
	 ...
	 * @param combox	Combox to send the response
	 * @param buffer	Buffer to send trough the combox
	 */
